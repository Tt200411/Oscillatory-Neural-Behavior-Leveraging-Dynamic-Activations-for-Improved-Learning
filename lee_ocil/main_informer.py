...
def main():
    ...
    for lee_type in range(9, 11):
        # 设置当前的lee_type
        config.lee_type = lee_type
    ...
    if config.lee_grid_search:
        ...
        setting = f'informer_{config.data}_lee{config.lee_type}_ft{config.features}_sl{config.seq_len}_ll{config.label_len}_pl{config.pred_len}_dm{config.d_model}_nh{config.n_heads}_el{config.e_layers}_dl{config.d_layers}_df{config.d_ff}_fc{config.factor}_eb{config.freq}_dt{config.distil}_test'
